...
const protect = handleAsyncError(async (req, res, next) => {
    ...
    //# iat: create a token, exp: expire time
    ...
    if (user.changedPasswordAfterCreatToken(decoded.iat)) {
...
const resetPassword = handleAsyncError(async function (req, res, next) {
    ...
    //? save dont need a validate because in user have everything
    //? and we explicitly written a password
    await user.save();
